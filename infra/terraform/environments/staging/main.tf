# CrestDesk - Staging Environment
#
# Staging environment with slightly larger instances than dev for
# realistic performance testing. Still cost-conscious where possible.
#
# Usage:
#   terraform init
...
module "vpc" {
  ...
  nat_gateway_count    = 1 # Single NAT for staging cost savings
...
module "ecs" {
  ...
  use_fargate_spot           = false # Standard Fargate for staging reliability
