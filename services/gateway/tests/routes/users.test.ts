...
describe('User routes — authentication', () => {
  ...
  it('GET /api/v1/users/me accepts valid token', async () => {
...
describe('User routes — authorization', () => {
  it('GET /api/v1/users requires managing_broker for listing all users', async () => {
    const res = await request(app)
      .get('/api/v1/users')
      .set('Authorization', authHeader(AGENT_USER));
    // agents can't list all users — either 403 or returns filtered
