/**
 * Security events route unit tests.
 */
...
describe('Security events — authentication', () => {
  it('GET /api/v1/security/events returns 401 without auth', async () => {
    const res = await request(app).get('/api/v1/security/events');
    expect(res.status).toBe(401);
  });

  it('GET /api/v1/security/events requires managing_broker', async () => {
