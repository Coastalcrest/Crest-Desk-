...
describe('Document routes — authentication', () => {
  it('GET /api/v1/documents returns 401 without auth', async () => {
    const res = await request(app).get('/api/v1/documents');
    expect(res.status).toBe(401);
  });

  it('GET /api/v1/documents accepts valid token', async () => {
    const res = await request(app).get('/api/v1/documents').set('Authorization', authHeader());
    expect(res.status).not.toBe(401);
  });
});

describe('Document routes — validation', () => {
  it('POST /api/v1/documents rejects empty body', async () => {
