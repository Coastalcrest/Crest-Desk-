...
describe('Commission routes — authentication', () => {
  it('GET /api/v1/commissions/structures returns 401 without auth', async () => {
    const res = await request(app).get('/api/v1/commissions/structures');
    expect(res.status).toBe(401);
  });

  it('GET /api/v1/commissions/structures accepts valid token', async () => {
    const res = await request(app).get('/api/v1/commissions/structures').set('Authorization', authHeader());
    expect(res.status).not.toBe(401);
  });
});

describe('Commission routes — authorization', () => {
  it('POST /api/v1/commissions/structures requires managing_broker', async () => {
    const res = await request(app)
      .post('/api/v1/commissions/structures')
      .set('Authorization', authHeader(AGENT_USER))
      .send({ name: 'Test', structureType: 'flat', tiers: [{ min: 0, max: null, rate: 70 }] });
