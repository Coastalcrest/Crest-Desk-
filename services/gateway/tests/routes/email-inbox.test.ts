/**
 * Email inbox route unit tests.
 */
import { describe, it, expect, vi } from 'vitest';
import request from 'supertest';
import app from '../../src/index';
import { authHeader } from '../utils/mock-auth';
...
describe('Email inbox routes — authentication', () => {
  it('GET /api/v1/email-inbox returns 401 without auth', async () => {
    const res = await request(app).get('/api/v1/email-inbox');
    expect(res.status).toBe(401);
  });

  it('GET /api/v1/email-inbox accepts valid token', async () => {
    const res = await request(app).get('/api/v1/email-inbox').set('Authorization', authHeader());
