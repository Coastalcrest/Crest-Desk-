...
describe('Social posts routes — authentication', () => {
  it('GET /api/v1/social-posts returns 401 without auth', async () => {
    const res = await request(app).get('/api/v1/social-posts');
    expect(res.status).toBe(401);
  });

  it('GET /api/v1/social-posts accepts valid token', async () => {
    const res = await request(app).get('/api/v1/social-posts').set('Authorization', authHeader());
